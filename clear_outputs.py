...
def clear_outputs(days: int = 0, dry_run: bool = False, keep_best: bool = False) -> Tuple[int, int, int]:
    ...
    # File extensions to always preserve (infrastructure files)
    preserve_extensions = {'.py', '.sh', '.md', '.txt', '.yml', '.yaml', '.j2', '.template'}
    
    # File patterns to preserve
    preserve_patterns = []
    if keep_best:
        preserve_patterns.extend(['best', 'final', 'top_'])
    
    # Files to always preserve by name
    preserve_files = {
        '.gitkeep', '.gitignore', 'README.md', 'requirements.txt',
        'job_info.txt', 'check_status.py', 'summary.log'
    }
    ...
    for output_dir in output_dirs:
        ...
        for root, dirs, files in os.walk(output_dir):
            ...
            for filename in files:
                ...
                # Check if file should be preserved by name
                if filename in preserve_files:
                    skipped_count += 1
                    continue
                
                # Check if file should be preserved by extension
                _, ext = os.path.splitext(filename)
                if ext.lower() in preserve_extensions:
                    print(f"  Preserving script/config: {file_path}")
                    skipped_count += 1
                    continue
                
                # Check if file should be preserved by pattern
                ...
                # Special handling for results directories - only delete data files
                if 'results' in output_dir or 'test' in output_dir:
                    # Only delete specific output file types in results directories
                    deletable_extensions = {'.json', '.csv', '.pdb', '.cif', '.sdf', '.mol2', 
                                           '.log', '.out', '.err', '.pkl', '.npy', '.npz'}
                    if ext.lower() not in deletable_extensions:
                        skipped_count += 1
                        continue
    ...
    for af_dir in alphafold_dirs:
        ...
        for protein_dir in os.listdir(af_dir):
            ...
            for item in os.listdir(protein_path):
                ...
                # Skip non-AlphaFold directories and files
                if not os.path.isdir(item_path):
                    # Check if it's a data file that should be deleted
                    _, ext = os.path.splitext(item)
                    if ext.lower() in {'.json', '.cif', '.pdb'}:
                        try:
                            ...
                            if days == 0 or file_mod_time < time_threshold:
                                if dry_run:
                                    print(f"  [Would delete] {item_path} ({format_size(file_size)})")
                                    deleted_count += 1
                                    total_size_freed += file_size
                                else:
                                    os.remove(item_path)
                                    print(f"  Deleted: {item_path} ({format_size(file_size)})")
                                    deleted_count += 1
                                    total_size_freed += file_size
                        except OSError as e:
...
If 0 or not specified, all outputs will be deleted."""
    ...
    if not args.confirm and not args.dry_run:
        ...
        print("  - Test results (data files only)")
        print("")
        print("The following will be preserved:")
        print("  - Python scripts (.py)")
        print("  - Shell scripts (.sh)")
        print("  - Configuration files (.yml, .yaml, .txt)")
        print("  - Documentation (.md)")
        print("  - Job monitoring files (check_status.py, job_info.txt)")
        print("")
        if args.keep_best:
            print("Additionally, files with 'best' or 'final' in the name will be preserved.")
        print("")
